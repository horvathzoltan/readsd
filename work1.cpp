...
int Work1::sha256sumDevice(const QString& fn, int r, qint64 b, const QString& sha_fn)
{
    ...
    auto cmd = QStringLiteral("/bin/sh -c \"sudo dd bs=%2 count=%3 if=%1 status=progress | sha256sum > %4\"").arg(fn).arg(r).arg(b).arg(sha_fn);
...
int Work1::dd(const QString& src, const QString& dst, int bs, int count, QString *mnt)
{
    QString e;
    //oflag sync
    auto cmd = QStringLiteral("sudo dd of=%1 bs=%3 count=%4 if=%2 status=progress conv=fdatasync").arg(dst).arg(src).arg(bs).arg(count);
    //zInfo(cmd);
    //return 1;
    auto out = Execute2(cmd);
